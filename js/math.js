// ============================================================
// MATH - normalCDF, percentile computation
// ============================================================
...
function computePercentile(item, pipelineData, normalityMap) {
  const normKey = item.CATEG_COD + '|' + item.SUBCAT_COD;
  const norm = normalityMap[normKey];
  const hasZ = item.Z_SCORE !== '' && item.Z_SCORE !== 'NULL'
               && item.Z_SCORE !== null && !isNaN(parseFloat(item.Z_SCORE));
  ...
  if (norm && norm.NORMAL === 'Yes') {
    return {
      pct: normalCDF(myZ),
      method: 'Z-table CDF (Shapiro-Wilk: normal distribution confirmed)'
    };
  }
  ...
  return {
    ...
    method: 'empirical rank (Shapiro-Wilk: non-normal distribution)'
  };
}
